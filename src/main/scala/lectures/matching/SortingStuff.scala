...
object SortingStuff extends App {
  ...
  case class StuffBox(books: List[Book] = Nil,
   ...
   // Поправьте логику метода
    ...
    private def putStuffInRightBox(item: Stuff, stuffBox: StuffBox): StuffBox = item match {
      ...
      case it@Boots(_, _) => stuffBox.copy(boots = it :: stuffBox.boots)
  ...
  // решение с функциями
  val sortJunkFunc = (stuff: List[Stuff]) => sort(stuff, StuffBox())
  val knifeIsInJunk2 = (sortJunkFunc andThen findMyKnife) (stuff)

//   val knifeIsInJunk = findMyKnife(sortJunk(stuff))


  println(s"Is knife in a junk? - $knifeIsInJunk")
  println(s"Is knife in a junk? - $knifeIsInJunk2")
}
